...
-(id)initWithFrameRect:(CGRect)rect ImageArray:(NSArray *)imgArr TitleArray:(NSArray *)titArr
{
	...
	if ((self=[super initWithFrame:rect]))
    {
        ...
        scrollView=[[UIScrollView alloc]initWithFrame:_viewSize];
        scrollView.pagingEnabled = YES;
        scrollView.contentSize = CGSizeMake(_viewSize.size.width * pageCount, _viewSize.size.height);
        ...
        //说明文字层
        UIView *noteView=[[UIView alloc] initWithFrame:CGRectMake(0, _viewSize.size.height-14,self.bounds.size.width,33)];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)_scrollView
{
    if (currentPageIndex==0) {
        ...
        [_scrollView setContentOffset:CGPointMake(([imageArray count]-2)*_viewSize.size.width, 0)];
    }
